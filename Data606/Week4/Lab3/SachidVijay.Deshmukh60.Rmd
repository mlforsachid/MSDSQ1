...
###**Date :** 10/05/2018
* > [GitHub Location for rmd file](https://github.com/mlforsachid/MSDSQ1/blob/master/Data606/Week4/Lab3/SachidVijay.Deshmukh60.Rmd)
* > [GitHub Location for pdf file](https://github.com/mlforsachid/MSDSQ1/blob/master/Data606/Week4/Lab3/SachidVijay.Deshmukh60.pdf)
* > [RPubs location of published file](http://rpubs.com/sachid/Data606Week4Lab)
...
* Male height is centered around 180 cm and female weight is centered around a65 cm. In general we can say that male height      is greater than female height
* Male height ranges from 160 to 200 cms
* Female height ranges from 150 to 180 cms
* Male height have more variance compared to female height
* 95% of the Males are with height range of 165 to 190 cms
* 95% of the female are in the height range of 155 to 175 cms
...
Answer : Based on the plot above we can see that data follows a bell shaped curve centered around mean 165. We can conculde that data follwos a nearly normal distribution
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?
```{r hist-heighsimone, eval=TRUE}
qqnorm(sim_norm)
```

Answer : Normal probability plot of sim_norm is very close with actual data. We can see that almost all the data points falls on the line. 
...
Answer: Normal probability plot for femal height looks similar to the plots created for simulated data. We can conclude that female heights are nearly normally distributed

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.

```{r hist-weightfemalesim, eval=TRUE}
qqnormsim(fdims$wgt)
```

Answer: Actual female weight data looks very similar to simulated weight data. Based on this we can conclude that frmale weight is nearly normally distributed
...
Question : - What is the probablity of male height between 180 cm to 190 cm?

```{r pnormmheight, eval=TRUE}
mhgtmean <- mean(mdims$hgt)
mhgtsd   <- sd(mdims$hgt)
pnorm(q = 190, mean = mhgtmean, sd = mhgtsd) - pnorm(q = 179, mean = mhgtmean, sd = mhgtsd) 
```

Answer : 38% males are having height between 180 to 190 cms

Question - What is the probability of female height less than or equal to 160 cms?

```{r pnorm, eval=TRUE}
pnorm(q = 160, mean = fhgtmean, sd = fhgtsd)
```

Answer : 22% females have height less than or equal to 160 cms
...
Answer: Stepwise normal curve indicates discrete variables. Plot D represent Age variable which is discrete. Plot C indicates female elbow whihc is more or less discrete (Follows uniform distribution)
...
Answer : Normal probability plot for female knee indicates skew on right side. We can see that points are deviating on normal probability plot from line in the end. This indicates right skewed data.
