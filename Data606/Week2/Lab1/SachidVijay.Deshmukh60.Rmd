...
###**Date :** 09/06/2018
* > [GitHub Location for rmd file](https://github.com/mlforsachid/MSDSQ1/blob/master/Data606/Week2/Lab1/SachidVijay.Deshmukh60.Rmd)
...
* > [RPubs location of published file](http://rpubs.com/sachid/Data606Lab1)
...
```{r WeightVsDesiredWeight, eval=TRUE}
plot(cdc$weight, cdc$wtdesire)
```

**We can see a positive correlation between actual weight and desired weight. However it can be observed
that there is not a linear relationship between them. Desired weight can take different values in a 
range for a given actual weight**
...
```{r variabletype, eval=TRUE}
typeof(wdiff)
```

* > wdiff is a numerical discrete type variable
* > If wdiff is 0 that means person's actual weight is same as desired weight. This indicates that 
person is in healthy state
* > Positive wdiff menas person is underweight
* > Negative wdiff means person is overweight
...
```{r wdiff dist, eval=TRUE}
boxplot(wdiff)
hist(wdiff)
```

**From the BoxPlot and Histogram, it can be seen that wdiff is normally distributed with center 0.
BoxPlot shows skew in the left. More people think they are overweight compare to people who think they are underweight.
Very few observations lies in 2nd and 3rd quarter in BoxPlot that means small number of people
think they are in healthy state.
There are some outliers in the data with very small or very high difference in the 
Desired Weight and Actual Weight.**

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
    
```{r weight compa, eval=TRUE}
boxplot(cdc$wtdesire-cdc$weight ~ cdc$gender)
```


**Feeling of overweight is same amongst male and females.
More males feel they are underweight compare to females.
More males think they are in healthy state compare to females.**
...
```{r weight mean, eval=TRUE}
mean.weight = mean(cdc$weight)
print(mean.weight)
```

```{r weight sd, eval=TRUE}
sd.weight = sd(cdc$weight)
print(sd.weight)
```
```{r weight prop, eval=TRUE}
lo = mean.weight - 0.5*sd.weight
hi = mean.weight + 0.5*sd.weight
total = length(cdc$weight)
percent = sum(cdc$weight >=lo & cdc$weight<=hi)/total *100
print(percent)
```
**39.335% weights are within one standard deviation of the mean**
