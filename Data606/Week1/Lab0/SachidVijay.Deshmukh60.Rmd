...
* > [GitHub Location for pdf file](https://github.com/mlforsachid/MSDSQ1/blob/master/Data606/Week1/Lab0/SachidVijay.Deshmukh60.pdf)
* > [RPubs location of published file](http://rpubs.com/sachid/Data606-Week1Assignment)
...
**Refernece library ggplot**

```{r refer-ggplot, eval=TRUE}
library("ggplot2")
```
...
* There is a strong negative trend for the number of girls baptized from year 1640 to 1660. 
* After 1660 there is a strong positive trend for the number of girls baptized.
* There is a noticiable spike just after year 1700 which shows steep deep in the number
 of girls baptized
...
```{r plot-boys-vs-year-line, eval=TRUE}
plot(x = arbuthnot$year, y = arbuthnot$boys, type = "l")
```

* The trend of boys baptized over years is similar to trend of girls baptized over years
* There is a strong negative trend for the number of boys baptized from year 1640 to 1660. 
* After 1660 there is a strong positive trend for the number of boys baptized.
* There is a noticiable spike just after year 1700 which shows steep deep in the number
 of boys baptized
...
```{r present-data-dim, eval=TRUE}
dim(present)
```

**what are the variable or column names?**
```{r present-data-col-names, eval=TRUE}
names(present)
```
**What years are included in this data set?**
```{r present-data-years, eval=TRUE}
paste("Year Range", min(present$year), "-", max(present$year))
```

**How do these counts compare to Arbuthnot's? Are they on a similar scale?**
```{r arbuthnot-present-count-comparison, eval=TRUE}
paste("Arbuthnot's Boys count range", min(arbuthnot$boys), "-", max(arbuthnot$boys))
paste("Arbuthnot's Girls count range", min(arbuthnot$girls), "-", max(arbuthnot$girls))
paste("Present's Boys count range", min(present$boys), "-", max(present$boys))
paste("Present's Girls count range", min(present$girls), "-", max(present$girls))
```
Looking at the min-max ranges for Boys and Girls count in Arbuthnot's dataset
and Present dataset, it can be concluded that coutns are not on similar scale

Boxplot below also confirms the same observation

```{r boxplot-present-count-comparison, eval=TRUE}
boxplot(present$boys, present$girls,arbuthnot$boys, arbuthnot$girls, col = c("orange","red"), names = c("Present Boys", "Present Girls", "Arbuthnot's Boys", "Arbuthnot's Girls"),horizontal = F,notch = TRUE)
```
...
```{r present-BoytoGirlRatio, eval=TRUE}
ggplot(present, aes(x = year)) + 
  geom_line(aes(y = boys), colour="blue") + 
  geom_line(aes(y = girls), colour = "red") + 
  ylab(label="Boys to Girl ratio") + 
  xlab("Year")
```


From the line chart it can be seen that boys being born are in 
greater poportion to girls being born every year in U.S.
This holds up Arbuthnot's observation in the study
performed in UK and applies that to U.S. as well.
...
```{r present-max year, eval=TRUE}
present$totalbirths = present$boys + present$girls

present.max.year = present$year[present$totalbirths==max(present$totalbirths)]

print(present.max.year)
```
